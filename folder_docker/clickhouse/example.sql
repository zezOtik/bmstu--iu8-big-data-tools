...
-- Ручная схема данных
CREATE TABLE default.taxi_info_2
(
    VendorID Int32,
...
INSERT INTO default.taxi_info_2
select * from default.taxi_info;
where 1 = 1;
...
FROM system.parts
WHERE table = 'taxi_info' AND active = 1
GROUP BY table;
...
CREATE TABLE taxi_info AS
SELECT *
FROM file('/var/lib/clickhouse/user_files/import/yellow_tripdata_2026-01.parquet', Parquet)
LIMIT 0;

select * from taxi_info;


-- Затем вставим данные:
INSERT INTO taxi_info
SELECT *
FROM file('/var/lib/clickhouse/user_files/import/yellow_tripdata_2025-12.parquet', Parquet);

CREATE TABLE taxi_sum
(
    VendorID Int32,
...
insert into taxi_sum
select
VendorID,
trip_distance
from taxi_info;

select * from taxi_sum;
...
INSERT INTO default.covid_stats
SELECT * FROM file(
  '/var/lib/clickhouse/user_files/import/countries-aggregated-iso.csv',
  'CSV',
  'date Date, country String, confirmed Int64, recovered Int64, deaths Int64'
);

;

truncate covid_stats;
select distinct country from default.covid_stats;
...
SELECT
    ...
    rows
FROM system.parts
WHERE (database = 'default') AND (`table` = 'taxi_info') AND active
ORDER BY name ASC;

-- Мутации
ALTER TABLE default.taxi_info UPDATE passenger_count = passenger_count + 2 WHERE TRUE;
