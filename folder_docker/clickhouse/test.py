...
# Полный маппинг: название → ISO 3166-1 alpha-2
COUNTRY_TO_ISO = {
    ...
    'Bulgaria': 'BG', 'Burkina Faso': 'BF', 'Burma': 'MM', 'Burundi': 'BI', 'Cabo Verde': 'CV',
    ...
    'Congo (Kinshasa)': 'CD', 'Costa Rica': 'CR', 'Cote d\'Ivoire': 'CI', 'Croatia': 'HR', 'Cuba': 'CU',
    'Cyprus': 'CY', 'Czechia': 'CZ', 'Denmark': 'DK', 'Diamond Princess': '--', 'Djibouti': 'DJ',
    ...
    'Kenya': 'KE', 'Korea, South': 'KR', 'Kosovo': 'XK', 'Kuwait': 'KW', 'Kyrgyzstan': 'KG',
    'Laos': 'LA', 'Latvia': 'LV', 'Lebanon': 'LB', 'Lesotho': 'LS', 'Liberia': 'LR',
    'Libya': 'LY', 'Liechtenstein': 'LI', 'Lithuania': 'LT', 'Luxembourg': 'LU', 'MS Zaandam': '--',
    ...
    'Togo': 'TG', 'Trinidad and Tobago': 'TT', 'Tunisia': 'TN', 'Turkey': 'TR', 'US': 'US',
    'Uganda': 'UG', 'Ukraine': 'UA', 'United Arab Emirates': 'AE', 'United Kingdom': 'GB', 'Uruguay': 'UY',
    'Uzbekistan': 'UZ', 'Vanuatu': 'VU', 'Venezuela': 'VE', 'Vietnam': 'VN', 'West Bank and Gaza': 'PS',
    'Yemen': 'YE', 'Zambia': 'ZM', 'Zimbabwe': 'ZW'
}

input_file = './import/countries-aggregated.csv'
output_file = './import/countries-aggregated-iso.csv'
...
with open(input_file, 'r', encoding='utf-8', newline='') as infile, \
    ...
    for row in reader:
        if len(row) >= 4:
            date, country, confirmed, recovered = row[0], row[1], row[2], row[3]
            deaths = row[4] if len(row) > 4 else '0'
            iso_code = COUNTRY_TO_ISO.get(country, country)
            writer.writerow([date, iso_code, confirmed, recovered, deaths])
